...
<p>An overview of the steps involved in the compilation and execution of the benchmarks follows.</p>

<p>We will consider the BSSN_PUGH:</p>

<p>1. Download the source code tarball:<code>
wget http://www.cactuscode.org/community/benchmarks/Bench_BSSN_PUGH.tar.gz
</code></p>

<p>2. Extract the contents of the tarball and move to the main Cactus directory:
...
<p>3. Create the configuration. It can be either: a standard configuration:
...
<code>make BSSN_PUGH_single-config options=&gt;&amp;OPTIONS&lt;&amp; THORNLIST=BENCHMARKS/Bench_BSSN_PUGH.thornlist</code></p>

<p>Depending on your operating system, you may need to use gtar and gmake instead of tar and make. The executable will be named BSSN_PUGH_single. is the name of the file containing the options. We used THORNLIST to specify the path to the thorn list.</p>
...
You can add -j3 at the end of the command to make the compilation faster (3 files at a time).</p>

<p>5. Run the benchmark. The benchmark is available in two sizes 80x80x80 and 100x100x100. To run it for the 100x100x100 case on a single processor machine and to redirect the result to a file called proc1.out use:
<code>./exe/cactus_BSSN_PUGH_single BENCHMARKS/Bench_BSSN_PUGH_100l.par &gt; proc1.out</code>
If you run the benchmark on a machine with multiple processors that doesn't have a queue, but has mpirun installed, use:
<code>mpirun -np 4 ./exe/cactus_BSSN_PUGH_single BENCHMARKS/Bench_BSSN_PUGH_100l.par &gt; proc1.out</code>
The proc1.out will include the results: the wall time usage the resource usage (rusage). The reported time for benchmarking is the wall time - from the last line: "Total time for simulation".</p>


<p>Below we have a screenshot of the output file:
<code>---------------------------------------------------------------
