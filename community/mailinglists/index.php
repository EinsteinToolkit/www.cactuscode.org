<?php $title='Mailing Lists';
include_once($_SERVER['DOCUMENT_ROOT'].'/global/header.php');?>

<!--
  <p>All public Cactus mailing lists   can be visited on <a href="http://www.cactuscode.org/mailman/listinfo" >http://www.cactuscode.org/mailman/listinfo</a>.</p>
-->

<p>
The <strong>news</strong> list is used to broadcast Cactus related events, interesting news and new releases.  The <strong>users</strong> list is a discussion forum for those using the standard releases of Cactus.  The <strong>developers</strong> list is a higher volume list for discussing new developments, details of patches, and much else.  The <strong>patch</strong> list should be used to submit patches to the Cactus team.
</p>

<ul>
  <dt><h4>News items:</h4></dt>
  <li>
    <a href="mailto:news@cactuscode.org">news@cactuscode.org</a>
    <br />
    [<a href="http://www.cactuscode.org/mailman/listinfo/news">subscribe</a>]
    &nbsp;
    [<a href="http://www.cactuscode.org/pipermail/news">archives</a>]
  </li>
  
  <br />
  <dt><h4>Discussion list for users:</h4></dt>
  <li>
    <a href="mailto:users@cactuscode.org">users@cactuscode.org</a>
    <br />
    [<a href="http://cactuscode.org/mailman/listinfo/users">subscribe</a>]
    &nbsp;
    [<a href="http://cactuscode.org/pipermail/users">archives</a>]
  </li>

  <br />
  <dt><h4>Discussion list for all developers:</h4></dt>
  <li>
    <a href="mailto:developers@cactuscode.org">developers@cactuscode.org</a>
    <br />
    [<a href="http://cactuscode.org/mailman/listinfo/developers">subscribe</a>]
    &nbsp;
    [<a href="http://cactuscode.org/pipermail/developers">archives</a>]
  </li>

  <br />
  <dt><h4>Cactus maintainer mailing list:</h4></dt>
  <li>
    <a href="mailto:cactusmaint@cactuscode.org">cactusmaint@cactuscode.org</a>
    <br />
 <!--   [<a href="http://www.cactuscode.org/mailman/listinfo/cactusmaint">subscribe</a>]
  --> &nbsp;
    [<a href="http://cactuscode.org/mailman/private/cactusmaint">archives</a>]
   </li>
  <p>This list is not public.  You can send questions, comments, or complaints to this list which you do not want to make in public.</p>

  <br />
  <dt><h4>Patch list:</h4></dt>
  <li>
    <a href="mailto:patches@cactuscode.org" >patches@cactuscode.org</a>
    <br />
    [<a href="http://cactuscode.org/mailman/listinfo/patches" >subscribe</a>] 
    &nbsp;
    [<a href="http://cactuscode.org/pipermail/patches" >archives</a>]
  </li>
  <p>This list is only for patches.  Anything sent to <em>patches</em> will be forwarded to the <em>developers</em> list with attachments removed, but a link left in so you can get the patch.  If you want to get the patch mailed to you, please subscribe to the <em>patches</em> mailing list.</p>
  <p>For more information on how to use this list, please read the <a href="/community/mailinglists/patches" >patches  information</a> page.</p>

  <br />
  <dt><h4>Commit message list:</h4></dt>
  <li>
  	<a href="mailto:commits@cactuscode.org" >commits@cactuscode.org</a>
  	<br />
    [<a href="http://cactuscode.org/mailman/listinfo/commits">subscribe</a>] 
  </li>

  <p>This mailing list carries the commit messages from
  svn.cactuscode.org.  You can subscribe to them if you want to stay
  up to date with Cactus development.  These lists are not meant for
  posting.  If you are interested in past commit messages, you can
  query the Subversion server, or also look at the
  [<a href="http://cactuscode.org/pipermail/commits" >archives</a>].</p>

<ul>


<?php include_once($_SERVER['DOCUMENT_ROOT'].'/global/footer.php');?>
